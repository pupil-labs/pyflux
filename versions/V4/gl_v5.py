...
from pyflux.poses import (
    PoseConverter,
    get_gaze_and_pose_df,
    rotation_matrix,
    get_pose_df_from_transforms_json,
)
...
print(recording_id)
strength = 0.0001
draw_gaze_circle = False
draw_depth_map = False
near_plane, far_plane = 0.01, 2.0

###########################################################################

record_flag = True
if record_flag:
    record_resolution = int(800 * 1600 / 1200), 800
    record_path = f"/home/kd/Desktop/{experiment}_gray_close.mp4"
...
global_cam.camera_pos = Vector3([0.0, -0.2, -0.5])
...
# df_gaze = get_pose_df_from_transforms_json(data_path / experiment / "transforms.json")
...
model = pyrr.matrix44.create_from_translation([0.0, 0.0, -0.25])
...
while not glfw.window_should_close(window.window):
    ...
    # update_global_cam()
    time = 3.8 + 0.0 * np.cos(0.4 * glfw.get_time())
    # time = 0.15 * glfw.get_time()
    ...
    for shader in [heatmap_shader]:
        ...
        shader._set_uniforms(
            model, pov_view, global_view, projection, strength=strength
        )
        for key, mesh in heat_tri_meshes.items():
