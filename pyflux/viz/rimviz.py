...
with open(os.path.join(Path(__file__).parent.parent, "config.json"), "r") as f:
    config = json.load(f)
    experiment_name = config["EXPERIMENT_NAME"]

ply_path = base_path / "exports" / experiment_name
...
if record_flag:
    ...
    record_path = f"{export_path}/{experiment_name}.mp4"
...
global_cam = Camera()
global_cam.camera_pos = Vector3([0.0, -2.5, -2.8])
...
###########################################################################
# Video resolution
width, height = 800, 600  # 1600, 1200
...
while not glfw.window_should_close(window.window):
    ...
    # Aerial view that rotates around the scene
    global_view = (
